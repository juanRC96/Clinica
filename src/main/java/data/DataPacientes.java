...
public class DataPacientes {
	...
	public LinkedList<Paciente> mostrarPacientes()
	{
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT dni,pnombre,papellido,fecha_nac,direccion,tel,osnombre FROM paciente pac\r\n"
	...
	public void agregarPaciente(Paciente p) throws Exception
	{
		...
		try {
			...
			con = DbConnector.getConexion();
			PreparedStatement ps =con.prepareStatement("INSERT INTO paciente(dni,pnombre,papellido,fecha_nac,direccion,tel,obra_social) " + "VALUES(?,?,?,?,?,?,?)");
	...
	public LinkedList<Paciente> buscarPacienteporDni(Paciente pac) throws Exception
	{
		...
		{
			con = DbConnector.getConexion();
			PreparedStatement ps =con.prepareStatement("SELECT dni,pnombre,papellido,fecha_nac,direccion,tel FROM paciente WHERE dni=?");
			ps.setInt (1, dni);
			rs = ps.executeQuery();
