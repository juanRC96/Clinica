...
public class DataAtencion {
	...
	public LinkedList<Atencion> mostrarAtenciones()
	{
		...
		try 
		{
			...
			PreparedStatement ps = con.prepareStatement("SELECT idAtencion,fecha,descripcion,observaciones,costo,oapellido,onombre,pac.dni,papellido,pnombre FROM atencion ate\r\n"
	...
	public void agregarAtencion(Atencion a) throws Exception
	{
		...
		try {
			...
			con = DbConnector.getConexion();
			PreparedStatement ps =con.prepareStatement("INSERT INTO atencion(dni,doctor,tratamiento,observaciones,fecha) " + "VALUES(?,?,?,?,?)");
